...
EndFunc   ;==>listConnectedPrinters

Func removePrinters()
	For $l = 0 To UBound($PrResult) - 1
		Local $resultWithoutWhitespace = StringStripWS($PrResult[$l], 2)
		Local $tmp = '"' & $resultWithoutWhitespace & '"'

;~ 		ConsoleWrite("$temp :" & $tmp & @CRLF)
		Local $command = "rundll32 printui.dll,PrintUIEntry /dn /n " & $tmp
		Run (@Comspec & " /c rundll32 printui.dll,PrintUIEntry /dn /n "&$tmp, "", @SW_Hide, 8)
